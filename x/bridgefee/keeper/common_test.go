...
func createTestInput(t *testing.T, isCheckTx bool) (sdk.Context, []auth.Account, Keeper) {
	...
	keyBridepool := sdk.NewKVStoreKey(bridgepoolTypes.ModuleName)
	keyBridefee := sdk.NewKVStoreKey(bridgefeeTypes.ModuleName)
	db := dbm.NewMemDB()
	ms := store.NewCommitMultiStore(db, false, 5000000)
	ms.MountStoreWithDB(keyAcc, sdk.StoreTypeIAVL, db)
	ms.MountStoreWithDB(keyBridepool, sdk.StoreTypeIAVL, db)
	ms.MountStoreWithDB(keyBridefee, sdk.StoreTypeIAVL, db)
	...
	keeper := NewKeeper(keyBridepool, cdc, ak, bridgefeeSubspace, "bridgefee")
	params := bridgefeeTypes.DefaultParams()
